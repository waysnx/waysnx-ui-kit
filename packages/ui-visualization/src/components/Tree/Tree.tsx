 ...
 * Expandable tree visualization — uses left-right or top-down layout.
 * Thinner than OrgChart: no minimap/toolbar by default.
 */
...
export function Tree({
  ...
  const resolvedAriaLabel = ariaLabel ?? t('visualization.tree.ariaLabel');
  const containerRef = useRef<HTMLDivElement>(null);

  const { nodes, layout, toggleExpand } = useVisualization(
    initialNodes,
    edges,
    { ...config, layout: { direction: 'left-right', ...config.layout } }
  );

  const { viewport, zoomIn, zoomOut, zoomToFit, onWheel, isPanning, onMouseDown, onMouseMove, onMouseUp } =

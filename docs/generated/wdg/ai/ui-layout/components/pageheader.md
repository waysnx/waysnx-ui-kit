# PageHeader

Display header information and controls at the top of a page.

## Purpose

Display header information and controls at the top of a page

## Installation

```bash
npm install @waysnx/ui-layout
```

## Import

```typescript
import { PageHeader } from '@waysnx/ui-layout';
```

## Props

| Prop | Type | Default | Required | Description |
|------|------|---------|----------|-------------|
| `children` | `React.ReactNode` | — | Yes |  |
| `label` | `string` | — | No |  |

## Design Tokens

### Spacing

| Token | CSS Properties | Selectors | Occurrences |
|-------|----------------|-----------|-------------|
| `--wx-spacing-md` | gap | .wx-page-header | 1 |

## Accessibility

**WCAG:** 2.1 AA

**Keyboard:** Unknown

**RTL:** Unknown

**High Contrast:** Unknown

## Storybook

[View Interactive Storybook](https://uikit.waysnx.tech/?path=/docs/components-pageheader)

## When to Use

Use this component when you need to:

- Use PageHeader for general-purpose components functionality

## When NOT to Use

No specific anti-patterns documented.

## Best Practices

**✓ Good use cases:**
- Use PageHeader for general-purpose components functionality

---

**Library:** `@waysnx/ui-layout`
**Category:** `components`
**Version:** `1.0.0`

## AI & Integration Notes

**Keywords:** label, components, pageheader

**Semantic Category:** utility

This component is indexed for AI agents, RAG pipelines, and documentation search.

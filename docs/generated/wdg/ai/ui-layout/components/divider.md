# Divider

Create a visual separation between content sections.

## Purpose

Create a visual separation between content sections

## Installation

```bash
npm install @waysnx/ui-layout
```

## Import

```typescript
import { Divider } from '@waysnx/ui-layout';
```

## Props

| Prop | Type | Default | Required | Description |
|------|------|---------|----------|-------------|
| `orientation` | `'horizontal' | 'vertical'` | — | No |  |
| `className` | `string` | — | No |  |

## Design Tokens

### Colors

| Token | CSS Properties | Selectors | Occurrences |
|-------|----------------|-----------|-------------|
| `--wx-color-border` | background-color | .wx-divider-horizontal, .wx-divider-vertical | 2 |

## Accessibility

**WCAG:** 2.1 AA

**Keyboard:** Unknown

**RTL:** Unknown

**High Contrast:** Unknown

## Storybook

[View Interactive Storybook](https://uikit.waysnx.tech/?path=/docs/components-divider)

## When to Use

Use this component when you need to:

- Use Divider for general-purpose components functionality

## When NOT to Use

No specific anti-patterns documented.

## Best Practices

**✓ Good use cases:**
- Use Divider for general-purpose components functionality

---

**Library:** `@waysnx/ui-layout`
**Category:** `components`
**Version:** `1.0.0`

## AI & Integration Notes

**Keywords:** components, divider

**Semantic Category:** utility

This component is indexed for AI agents, RAG pipelines, and documentation search.
